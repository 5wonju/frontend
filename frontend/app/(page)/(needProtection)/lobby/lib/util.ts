import { gameMode, IRoomInfo, IRoomOfLobby, ProblemCategoryType } from './type'

export const CATEGORY_LIST: ProblemCategoryType[] = ['개발', '과학', '컴퓨터', '한국사', '근현대사']

// api 연결 전에 더미 방 리스트 데이터 생성
// export const generateRooms = (): IRoomOfLobby[] => {
//   const rooms: IRoomOfLobby[] = []
//   for (let i = 1; i <= 100; i++) {
...
//       totalRound: 20,
//       roomMode: ['BASIC', 'YOOT'][Math.floor(Math.random() * 2)] as gameMode,
//     })
//   }
...
export const isWaitingRoomData = (room: any): room is IRoomInfo => {
  return (
...
// - 입장 가능한 방인지 검사
export const canEnterRoom = (room: IRoomOfLobby) => {
...
export const validateCreateRoomData = ({
  ...
  if (roomPW && roomPW.length > 0 && roomPW.length < 4) {
